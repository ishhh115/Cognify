...
export const getAllFlashcardSets = async (req, res, next) => {
    try {
        const flashcardSets = await Flashcard.find({ userId: req.user._id, documentId: req.params.documentId })
        .populate('documentId', 'title fileName')
        .sort({ createdAt: -1 });
...
//@desc Mark flashcared as reviewed
//@route POST /api/flashcards/:cardId/review
//@access Private
export const reviewFlashcardSet = async (req, res, next) => {
    try {
...
//@desc Toggle star/favorite on flashcard set
//@route PUT /api/flashcards/:cardId/star
//@access Private
export const toggleFlashcardSet = async (req, res, next) => {
    try {
        const flashcardSet = await Flashcard.findOne({
            'cards._id': req.params.cardId,
            userId: req.user._id
